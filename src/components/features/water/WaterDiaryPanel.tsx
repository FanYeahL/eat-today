 ...
 * 水占·干饭日记面板
 * ─────────────────────────────────────────────
 * 把抽取玩法的 DiaryView 原样搬进水占：DiaryView 本就用全站浅色 token
 * （天空蓝/草木绿），不需重画配色——这里只负责一层「浮在水面上」的
 * 半透明卡壳 + 滚动容器，让它落进水占的清冷氛围里。
 *
 * 数据骨干仍是 lib/diary：水占「循此而去」写入的记录，
 * 这里直接读得到，两个入口共用同一本日记。
 */
...
export default function WaterDiaryPanel({ onClose }: WaterDiaryPanelProps) {
  return (
    <motion.div
      ...
      className="water-panel-overlay absolute inset-0 z-50 flex justify-center overflow-y-auto px-5 pb-12 pt-24"
    >
